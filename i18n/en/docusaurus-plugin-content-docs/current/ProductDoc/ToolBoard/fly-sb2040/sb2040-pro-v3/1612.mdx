...
##### Add the following configuration to the `printer.cfg` configuration file to debug Eddy.
    ...
    ## Mesh Bed Commands
        ...
        * Add the following macro to the `printer.cfg` configuration.
            ...
            ```
        * For the local area network mesh bed command, it is necessary to ensure that Klipper can use the object exclusion function. It is recommended to add it to the startup macro.
            ```bash
...
### bed_mesh Configuration
    ...
    * In the `[bed_mesh]`, it is recommended to set the `horizontal_move_z` parameter to 2 to make Eddy as close to the platform as possible when scanning the platform.
...
## Anti-collision Macro
    ...
    ### `[quad_gantry_level]` Bed Sweeping Optimization
        ...
        * This macro will reduce the detection height as the gantry height difference decreases.
            ```cfg
   ...
   ## The First Step: Modify the Frequency
    ...
    This method is only used for sht36-v3/SB2040-v3 or SB2040-Pro-v3.

...
## Start Compilation
    ...
    * Press `Q`, then a **Save configuration** prompt will appear, press `Y` again at this time
...
## Burn Firmware
    ...
    * Use an XT30 data cable to connect the tool board to UTOC or the main board with bridge firmware installed before burning the firmware, refer to the wiring diagram below
    ...
    * Please ensure that the resistance between CAN H and CAN L is around `60Ω` when measured with a multimeter under power-off condition, if it is `120Ω`, you need to turn on the `120Ω` DIP switch, if it is around `40Ω`, you need to turn off the DIP switch

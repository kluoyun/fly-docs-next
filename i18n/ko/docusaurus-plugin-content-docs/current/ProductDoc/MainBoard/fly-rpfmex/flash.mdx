---
sidebar_position: 2
sidebar_label: 펌웨어 플래싱
---

import Katapult from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/dfu-flash/_host-katapult-flash.mdx';
import MakeOperate from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/klipper-make/_make-operate.mdx';
import USB from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/dfu-flash/_firmware/2040/_usb1.mdx';
import SSH from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/dfu-flash/_firmware/_ssh.mdx';

import LS from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/dfu-flash/_usb.mdx';

# USB 펌웨어 컴파일

## 컴파일 시작

    <SSH />

    <MakeOperate/>

    <USB />

    * `Q` 키를 누르고 **Save configuration**이 나타날 때까지 기다린 후 `Y` 키를 누릅니다.
    <ImageView image={require('@site/docs/General/dfu-flash/img/configuration.webp').default} size="50%" align="center" />
    * 이제 설정이 저장되었으며 명령줄 인터페이스로 돌아왔습니다.
    * `make -j4`를 입력하여 컴파일을 시작합니다. 시간이 다소 걸릴 수 있습니다.
    <ImageView image={require('@site/docs/General/dfu-flash/img/make.webp').default} size="50%" align="center" />
    * 아래 내용이 출력되면 컴파일이 성공적으로 완료되었습니다.

        ```bash
        Linking out/klipper.elf
        Creating hex file out/klipper.bin
        ```

## 플래싱 모드 확인

    * 상위 컴퓨터가 정상적으로 네트워크에 연결되어 있는지 확인하고, 하위 컴퓨터와 데이터 전송 기능이 있는 데이터 케이블이 상위 컴퓨터에 연결되어 있는지 확인하십시오.
    * 상위 컴퓨터에 SSH 연결 후 `lsusb`를 입력하고 Enter 키를 누릅니다. 아래 이미지에서 빨간색으로 표시된 정보가 나타나면 다음 단계로 진행할 수 있습니다.
        * 아무런 정보도 반환되지 않는 경우, 이는 상위 컴퓨터 시스템의 문제입니다. 해결 방법은 없으므로 정상적인 시스템으로 교체하거나 상위 컴퓨터를 교체해야 합니다.
        * `lsusb` 명령이 없다는 메시지가 표시되면 다음 명령어를 실행하여 설치할 수 있습니다.
            ```bash
            sudo apt-get install usbutils
            ```
            <ImageView image={require('@site/docs/ProductDoc/MainBoard/fly-micro/fly-micro4/flash/img/6177.webp').default} size="70%" align="center" /> 
    * 빨간색 사각형 내부에 표시된 ID는 참고용이며 고정된 값은 아닙니다. `OpenMoko, Inc. rp2040` 이후의 접미사 `XX`는 가변적입니다.
        * 일부 상위 컴퓨터는 시스템 문제로 인해 정보가 제대로 표시되지 않을 수 있습니다.

## 펌웨어 플래싱


    <Katapult />

...
## バーンモードへの移行
    ...
    * 主基板を上位機にType-Cケーブルで接続し、**リセットボタンをダブルクリック**すると、主基板上のLEDが点滅します。
    :::warning 警告
       * LEDが点滅しない場合、katapultファームウェアの再焼付けが必要です：<Button variant="contained" disableElevation href="/docs/ProductDoc/MainBoard/fly-d/fly-d5/flash/bl#進入dfu燒錄模式">katapultファームウェアの焼付け</Button>

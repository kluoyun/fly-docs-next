---
sidebar_position: 2
sidebar_label: 펌웨어 프래싱
---

import Katapult from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/dfu-flash/_host-katapult-flash.mdx';
import MakeOperate from "/docs/General/klipper-make/_make-operate.mdx";
import USB from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/dfu-flash/_firmware/2040/_usb.mdx';
import SSH from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/dfu-flash/_firmware/_ssh.mdx';

import LS from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/dfu-flash/_usb.mdx';

# USB 펌웨어 컴파일

    :::tip

    * USB 가속계는 USB 연결된 Katapult 펌웨어로 공장에서 미리 프래싱됩니다.
    * 문제가 발견되면 직접 프래싱할 수 있습니다.
    * [Katapult 펌웨어 프래싱](./bl.mdx "클릭하여 이동")

    :::

## 빌드 시작

    <SSH />

    <MakeOperate/>

    <USB />
    * 마이크로 컨트롤러 시작 시 GPIO 핀을 설정하려면, gpio24 입력:
        <ImageView image={require('@site/docs/ProductDoc/ToolBoard/fly-usb-adxl/fly-usb-lis2dw/img/usb.webp').default} size="100%" align="center" />
    * `Q` 키를 누르면, **Save configuration**이 나타나고, 그 다음에 `Y` 키를 누릅니다.
    <ImageView image={require('@site/docs/General/dfu-flash/img/configuration.webp').default} size="50%" align="center" />
    * 이제 설정이 저장되고 명령줄 인터페이스로 돌아왔습니다.
    * `make -j4`를 입력하여 빌드를 시작합니다. 시간이 좀 걸립니다.
    <ImageView image={require('@site/docs/General/dfu-flash/img/make.webp').default} size="50%" align="center" />
    * 마지막으로 아래의 내용이 출력되면 빌드가 성공했습니다.

        ```bash
        Linking out/klipper.elf
        Creating hex file out/klipper.bin
        ```

## 장치 검색

    <LS />


## 펌웨어 프래싱


    <Katapult />


## 펌웨어 시작 확인

    * 펌웨어가 정상적으로 시작하면 LED가 켜집니다.
    <ImageView image={require('@site/docs/ProductDoc/ToolBoard/fly-usb-adxl/fly-usb-lis2dw/img/led.webp').default} size="100%" align="center" />
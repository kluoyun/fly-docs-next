---
sidebar_position: 8
sidebar_label: Вентилятор на плате 5В
---




# Вентилятор на плате 5В

## Плата Pi-V2 имеет контролируемый разъем напряжением 5В для охлаждения основного процессора. Схема подключения приведена на рисунке ниже.
    <ImageView image={require('@site/docs/ProductDoc/SBC/fly-pi-v2/img/fan.webp').default} size="80%" align="left" />


## Пример конфигурации

    ```cfg
    [mcu host]                      # FlyOS автоматически создал linux MCU, достаточно его определить для использования           
    serial: /tmp/klipper_host_mcu 

    [temperature_sensor FLY-π]      # Температура с верхнего уровня
    sensor_type: temperature_host


    [temperature_fan core_fan]      # Вентилятор охлаждения верхнего уровня
    pin: host:gpiochip1/gpio103     # Ножка вентилятора верхнего уровня
    max_power: 1.0
    sensor_type: temperature_host   # Установлено как температура основного контроллера верхнего уровня
    control:watermark               # Способ управления
    target_temp: 48                 # Температура запуска вентилятора верхнего уровня
    min_temp: 0                     # Минимальная температура, ниже которой будет выдана ошибка
    max_temp: 90                    # Максимальная температура, выше которой будет выдана ошибка
    off_below: 0.10
    kick_start_time: 0.50
    max_speed: 0.8                  # Максимальная скорость, равная 80% от полной мощности
    min_speed: 0.3                  # Минимальная скорость, равная 30% от полной мощности
    ```

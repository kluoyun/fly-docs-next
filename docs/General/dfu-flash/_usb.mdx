    ...
    * `1d50:6177` 属于本次你要用到的设备。`OpenMoko, Inc. stm32f072xb`后缀具有一定参考价值，但不作为判断依据。
        * 部分上位机可能因为系统问题，显示不全或者不显示。
        * 如果输入`lsusb`指令能够显示设备，但是没有 `1d50:6177`，请尝试更换数据线，尝试将主板连接到上位机的其他USB口。
    * 如果您的之前执行过上述步骤，并且成功烧录Klipper固件，主板运行正常，只是更新Klipper固件，请查看：<Button variant="contained" disableElevation href="/docs/ProductDoc/MainBoard/fly-d/fly-d5/flash/usb#更新固件方法">Klipper固件更新</Button>

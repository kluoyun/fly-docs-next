...
## slicer 起始コード は bilibili のアップローダー muimoon女生が提供しました:
```bash
...
コピーまたはミラーマクロを再利用するには、スライサー内で対応するコードを参照する必要があります

* ACTIVATE_COPY_MODE コピー モード
* ACTIVATE_MIRROR_MODE ミラー モード
...
# ヘルパー スクリプトでキャリッジを駐車する（T0 および T1 マクロから呼び出されます）

...
## SSH 연결하기
    ...
    :::danger 경고
    - 펌웨어를 컴파일하기 전에 `와이파이를 사용하여 상위 컴퓨터로 SSH 연결`해야 합니다.
    :::

 * 먼저 다음을 수행하세요: <Button variant="contained" disableElevation component="a" rel="noopener" href="/docs/ProductDoc/MainBoard/fly-c/fly-c5/ssh#사용-wifi-connect-to-up-computer-ssh">와이파이를 사용하여 상위 컴퓨터로 SSH 연결</Button>  
 ---
 <br/>
...
<Tabs>
    ...
    ### USB 브리지 CAN 펌웨어 플래시
    :::tip 팁
    - FLYOS-FAST 시스템에는 미리 컴파일된 USB 브리지 CAN 펌웨어가 포함되어 있으므로 별도로 펌웨어를 컴파일할 필요가 없습니다. 아래 명령을 실행하면 플래시할 수 있습니다.
    - 또한 `수동으로 USB 브리지 CAN 펌웨어 컴파일`을 선택하여 펌웨어를 직접 컴파일할 수 있습니다.
    :::

    * 장치에 따라 신중히 선택하십시오. 기본값은 `1M`입니다.

    * C5 시스템의 기본 비트레이트는 1M입니다. 500K를 사용하려면 시스템 설정에서 `canbus_bitrate`의 비트레이트를 수정해야 합니다.

    * 이 명령은 `1M` USB 브리지 CAN 펌웨어를 플래시합니다.
    ```bash
    fly-flash -d c5 -h -f /usr/lib/firmware/klipper-h723-128k-usbcan-1m.bin
    ```
    * 이 명령은 `500k` USB 브리지 CAN 펌웨어를 플래시합니다.
    ```bash

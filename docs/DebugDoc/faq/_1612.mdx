...
:::
* 第一步
* 打印提供的高度模型<Button variant="contained" disableElevation href="http://cdn.mellow.klipper.cn/STL/Current%20Height%20Test%20Model.stl">高度模型</Button>
* 点击如图片显示的宏命令`CALIBRATE_DD`点击执行
* 通过控制台控制z轴离热床大概3cm-5cm的高度拿出打印好的高度模型放到工具头下方 然后继续控制z轴,直到压到高度模型的中心点圆点，然后继续按照提示操作
...
## 数值没有变化调整命令
* 通过下面命令去调整Z—offset 弹出来的UI界面 调整`z—offset`调整的数值就是打印过程中调整的数值[Paper test](https://www.klipper3d.org/Bed_Level.html#the-paper-test)
```bash
G28

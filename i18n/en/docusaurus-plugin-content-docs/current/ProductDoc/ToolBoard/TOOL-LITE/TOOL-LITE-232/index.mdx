...
FLY TOOL-LITE-232 is a tool board designed by Guangzhou Maglun Electronic Technology Co., Ltd. for 3D printers. This tool board provides an xt30/kf interface, which can replace the complicated wiring between the original motherboard and the print head with four wires, optimizing the layout. TOOL-LITE-232 is suitable for 36/42 stepper motors and integrates an RS232 transceiver, TypeC port, TMC2209 driver, LIS2DW accelerometer, 2 controllable fans, and other features. This tool board is compatible with Klipper firmware.
...
## 1.1 Product Features
    ...
    * A Boot button is reserved, and users can enter Boot mode to update the firmware via TypeC
    * Supports Klipper firmware
    * Supports S232 mode
    * In RS232 mode, it has a higher transmission rate and stronger stability.
...
## 1.2 New Features
    ...
    * Added a 1-meter power/signal connection cable, the cable uses a twisted pair design for stronger anti-interference performance

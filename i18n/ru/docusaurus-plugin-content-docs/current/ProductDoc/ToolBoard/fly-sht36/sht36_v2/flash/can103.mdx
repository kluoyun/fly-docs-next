...
## Начало компиляции
    ...
    * Теперь конфигурация должна быть сохранена, и вы должны оказаться в командной строке
...
## Запись прошивки
    ...
    * Перед записью прошивки подключите инструментальную плату к UTOC или материнской плате с прошивкой моста с помощью провода XT30. Ниже приведен參考ный рисунок соединения
    ...
    * Проверьте сопротивление между CAN H и CAN L при выключенном питании. Оно должно быть около `60 Ом`. Если показывает `120 Ом`, подключите переходник `120 Ом`. Если около `40 Ом`, удалите переходник

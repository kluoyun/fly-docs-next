...
## 将传感器电缆连接到主板的EXP1接口
    *如传感器的电缆线距不够可以使用包装里面延迟线
    * BDsensor-m的CKL和SDA线可以连接到电路板的任何GPIO引脚。您也可以将BD传感器电缆直接连接到Bltouch端口，例如：
    ...
    * 由于主板连接器中的某些引脚可能没有直接连接到MCU的gpios（例如，它们上可能有滤波电容器或通过MOSFET，二极管或光耦合器隔离，但如果它们通过电阻或电阻上拉/下拉隔离也可以），因此它们不能与`BDsensor-m`一起使用。并且固件将报告连接错误。例如
...
## 将补丁安装到 klipper 固件中
    ...
    * 然后执行下方命令进行安装
        ...
        ./install_BDsensor.sh
        ```
### 这里的编译固件`如提示上下位机固件不一致`才进行编译新的klipper固件
    * 编译固件找到自己固件的编译教程进行编译刷写
    * 编译固件   

        ```bash
        cd ~/klipper/  #转到klipper目录
        make menuconfig  #进入klipper编译界面命令
        make clean  #清理命令
        make   #编译命令
        ```
...
## 校准
    ...
    * 清洁喷嘴，然后控通过控制台移动Z轴，直到喷嘴刚好接触床板（BDsensor-m将使用此位置作为零位置，因此不需要`z_offset`，这就是[BDsensor-m]部分中值为0的原因）

...
## 컴파일 시작
    ...
    * **마이크로컨트롤러 시작 시 설정할 GPIO 핀**을 선택하고 입력: **!gpio5**
    :::warning
...
## 펌웨어 플래싱
    ...
    * 펌웨어 플래싱 전에 XT30 데이터 케이블을 도구 보드를 UTOC 또는 브리지 펌웨어가 설치된 메인보드에 연결합니다. 참고 연결도를 아래 참조하십시오.
    ...
    * CAN H와 CAN L가 전원이 꺼진 상태에서 multimeter를 사용하여 저항 값을 약 `60Ω`으로 측정하는지 확인하십시오. 만약 `120Ω`이면 `120Ω` 스위치를 켜야 하며, 약 `40Ω`이면 스위치를 끄십시오.

---
sidebar_position: 3
sidebar_label: CAN固件烧录
---

import CANFLASH from '@site/i18n/fr/docusaurus-plugin-content-docs/current/General/dfu-flash/_host-can-flash.mdx';
import MakeOperate from "/docs/General/klipper-make/_make-operate.mdx";
import CAN from '@site/i18n/fr/docusaurus-plugin-content-docs/current/General/dfu-flash/_firmware/2040/_can1.mdx';
import SSH from '@site/i18n/fr/docusaurus-plugin-content-docs/current/General/dfu-flash/_firmware/_ssh.mdx';

import LS from '@site/i18n/fr/docusaurus-plugin-content-docs/current/General/dfu-flash/_can.mdx';

# CAN 固件编译

## 开始编译

    <SSH />

    <MakeOperate/>

    <CAN />

    * Sélectionnez **GPIO pins to set at micro-controller startup**, entrez : **!gpio5**
    :::warning
        Veuillez entrer `!gpio5` en mode d'entrée anglaise.
    :::
        <ImageView image={require('@site/docs/ProductDoc/ToolBoard/fly-sb2040/sb2040-pro-v3/flash/img/can.webp').default} size="100%" align="center" />
    * Appuyez sur la touche `Q`, puis apparaît **Save configuration**, appuyez à nouveau sur `Y`
    <ImageView image={require('@site/docs/General/dfu-flash/img/configuration.webp').default} size="50%" align="center" />
    * Vous devriez maintenant avoir sauvegardé la configuration et être revenu à l'interface de ligne de commande
    * Tapez `make -j4` pour commencer la compilation, cela prend un certain temps
    <ImageView image={require('@site/docs/General/dfu-flash/img/make.webp').default} size="50%" align="center" />
    * Si la sortie finale ressemble au texte ci-dessous, la compilation a réussi.

        ```bash
        Linking out/klipper.elf
        Creating hex file out/klipper.bin
        ```

## Recherche de périphériques

    <LS />


## Écriture du firmware

    * Avant d'écrire le firmware, connectez la carte de l'outil au UTOC ou à la carte mère flashée avec le firmware de pont à l'aide d'un câble XT30. Le schéma de câblage est fourni en référence ci-dessous
    * Assurez-vous que les commutateurs sont positionnés correctement
        <ImageView image={require('@site/docs/ProductDoc/ToolBoard/fly-sb2040/sb2040-v3/flash/img/sb2040v3_utoc.webp').default} size="80%" align="left" />
    * Assurez-vous que la résistance entre CAN H et CAN L est d'environ `60Ω` lorsqu'il n'y a pas d'alimentation électrique. Si elle est d'environ `120Ω`, activez le commutateur de résistance de `120Ω`. Si elle est d'environ `40Ω`, désactivez le commutateur.
        <ImageView image={require('@site/docs/ProductDoc/ToolBoard/fly-sb2040/sb2040-v3/flash/img/120.webp').default} size="100%" align="center" />

    <CANFLASH />

## Vérification du démarrage du firmware

    * Si le firmware démarre normalement, la LED s'allumera.
        <ImageView image={require('@site/docs/ProductDoc/ToolBoard/fly-sb2040/sb2040-pro-v3/flash/img/led.webp').default} size="100%" align="center" />
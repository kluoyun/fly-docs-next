    ...
    ### 第二步：校准EDDY
        ...
        * 然后输入下面命令
        ```bash
            G28 X Y
        ```
        * 执行下面命令，控制打印头到热床中央2cm高的位置
        ```bash
            SET_KINEMATIC_POSITION x=50 y=50 z=150
            LDC_CALIBRATE_DRIVE_CURRENT CHIP=fly_eddy_probe
            PROBE_EDDY_CURRENT_CALIBRATE CHIP=fly_eddy_probe
        ```
        * 执行了会有一个弹窗出来做抽纸动作（也可以垫A4纸确保间隙合适）[Z轴偏移校准（Paper Test）方法](https://www.klipper3d.org/Bed_Level.html#the-paper-test)

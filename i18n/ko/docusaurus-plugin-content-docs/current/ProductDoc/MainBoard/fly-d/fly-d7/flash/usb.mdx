...
## 플래싱 모드 진입
    ...
    :::
    * 데이터 케이블로 메인보드를 상위 기계에 연결한 후, **리셋 버튼을 두 번 클릭**하면 메인보드의 LED가 깜빡입니다.
    :::warning 경고
       * 만약 LED가 깜빡이지 않는다면 Katapult 펌웨어를 다시 플래시해야 합니다: <Button variant="contained" disableElevation component="a" rel="noopener" href="/docs/ProductDoc/MainBoard/fly-d/fly-d7/flash/bl#enter-dfu-flash-mode">Katapult 펌웨어 플래시</Button>
       * LED가 깜빡이지 않는 경우 Katapult 펌웨어 플래시 후 다시 진행하세요.
    :::

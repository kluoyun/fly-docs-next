...
## 부트로더 모드 진입
    ...
    * 메인보드를 상위 기계에 TypeC 데이터선으로 연결한 후, **리셋 버튼을 두 번 클릭**합니다. 이때 메인보드의 LED가 깜빡입니다.
    :::경고
       * 만약 LED가 깜빡이지 않는다면 Katapult 펌웨어를 다시 써야 합니다: <Button variant="contained" disableElevation href="/docs/ProductDoc/MainBoard/fly-d/fly-d7/flash/bl#進入dfu燒錄模式">Katapult 펌웨어 쓰기</Button>

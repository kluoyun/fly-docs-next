...
## Screen Wiring
    <Tabs>
        ...
        :::tip
        - The package of the TFT screen will come with two FPC cables, one is `14P (narrow)` and the other is `16P (wide)`
        - Please make sure to use the `14P (narrow)` for the resistive screen!!!
        :::
...
<Tabs>
    ...
    :::info
    - After connecting the host machine to WiFi and obtaining its IP address, enter `IP+:9999` in the browser to access FLY TOOLS for screen-related configurations
    - For example, if your host machine's IP address is `192.168.6.110`, you need to enter `192.168.6.110:9999` in the browser
    :::
    1. Open FLY TOOLS, follow the steps shown in the diagram below to `enable Klipper Screen`, `select the screen model`, and click the pink button `Save Configuration` at the bottom right. **Restarting the host machine will take effect!**
      :::warning
      - For `resistive screens`, select `fly-tft-v1`. Make sure the DIP switch on the back of the screen is set to `Resi`
      - For `capacitive screens`, select `fly-tft-v2-c`. Make sure the DIP switch on the back of the screen is set to `Cap`
      :::
      <table>
        <tr>
          <td>
...
<br />
<br />
    2. You can also configure it directly via command line in SSH (not recommended!)
        * To configure a `resistive screen`, use the following command to set it to `fly-tft-v1`. Make sure the DIP switch on the back of the screen is set to `Resi`
        ```cfg
        sudo sed '11s/screen=none/screen=fly-tft-v1/' -i /boot/FlyOS-Env.txt
        ```

        * To configure a `capacitive screen`, use the following command to set it to `fly-tft-v2-r`. Make sure the DIP switch on the back of the screen is set to `Cap`
        ```cfg
    ...
    1. Open the device IP address in the browser, for example: `http://192.168.6.110/` 
       <table>
         <tr>
           <td>
             In `fluidd`, as shown in the figure below: `uncheck` -> `filter hidden files and folders`.
             <ImageView image={require('@site/docs/DebugDoc/flyos-fast/img/config1.webp').default} size="100%" align="center" />
           </td>
           <td>
             In `mainsail`, as shown in the figure below: `check` -> `show hidden files`.
             <ImageView image={require('@site/docs/DebugDoc/flyos-fast/img/config2.webp').default} size="85%" align="center" />
           </td>
         </tr>
       </table>
  
    2. At this point, you can see the `.flyos-config` folder. Entering this folder will have a `sys-config.conf` file.

     <ImageView image={require('@site/docs/DebugDoc/flyos-fast/img/config3.webp').default} size="60%" align="center" />
    
    3. Open the `sys-config.conf` file, modify the required configuration items, then `save` and `close` it.
    :::warning
        - For `resistive screens`, configure as `fly-tft-v1`. Please note that there should be **no spaces** after `=`. Make sure the DIP switch on the back of the screen is set to `Resi`
        - For `capacitive screens`, configure as `fly-tft-v2-c`. Please note that there should be **no spaces** after `=`. Make sure the DIP switch on the back of the screen is set to `Cap`
    ...
    4. After modifying and saving, `restart the system` to take effect.
    </TabItem>
</Tabs>    

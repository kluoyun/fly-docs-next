 ...
 * 最初に：<Button variant="contained" disableElevation component="a" rel="noopener" href="/docs/ProductDoc/MainBoard/fly-c/fly-c5/ssh#使用wifi接続して上位機にSSH接続">wifiで上位機にSSH接続</Button>  
 ---
 <br/>
...
<Tabs>
    ...
    - 「USBブリッジCANファームウェアの手動コンパイル」を選択することもできます。
    :::

    * お使いのデバイスに応じて適切に選択してください。FLYシステムのデフォルトは`1M`です。

    * C5システムのデフォルトは1Mレートですが、500Kにする場合はシステム設定で`canbus_bitrate`のレートを変更してください。

    * このコマンドは`1M`の`USBブリッジCANファームウェア`を書き込みます
    ```bash
    fly-flash -d c5 -h -f /usr/lib/firmware/klipper-h723-128k-usbcan-1m.bin
    ```
    * このコマンドは`500k`の`USBブリッジCANファームウェア`を書き込みます
    ```bash
    ...
    ---
        ...
        ### ファームウェアの書き込み
        :::tip
        ファームウェアを書き込むには以下のコマンドを実行してください
        :::

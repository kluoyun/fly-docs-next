---
sidebar_position: 2
sidebar_label: Programmeur USB

---

import Katapult from '@site/i18n/fr/docusaurus-plugin-content-docs/current/General/dfu-flash/_host-katapult-flash.mdx';
import MakeOperate from "/docs/General/klipper-make/_make-operate.mdx";
import USB from '@site/i18n/fr/docusaurus-plugin-content-docs/current/General/dfu-flash/_firmware/2040/_usb.mdx';
import SSH from '@site/i18n/fr/docusaurus-plugin-content-docs/current/General/dfu-flash/_firmware/_ssh.mdx';

import LS from '@site/i18n/fr/docusaurus-plugin-content-docs/current/General/dfu-flash/_usb.mdx';
import CANBOOT from '@site/i18n/fr/docusaurus-plugin-content-docs/current/General/dfu-flash/_host-katapult-flash.mdx';

# Compilation du firmware USB

    :::tip

    * La Micro4 est préchargée avec le programme Katapult via USB, il n'est pas nécessaire de la reprogrammer.

    :::

## Démarrer la compilation

    <SSH />

    <MakeOperate/>

    <USB />

    * Sélectionner les broches GPIO à configurer au démarrage du microcontrôleur, entrer : gpio8
        <ImageView image={require('@site/docs/ProductDoc/MainBoard/fly-micro/fly-micro4/flash/img/usb.webp').default} size="100%" align="center" />
    * Appuyer sur la touche `Q`, apparaît **Save configuration**, puis appuyer sur `Y`
        <ImageView image={require('@site/docs/General/dfu-flash/img/configuration.webp').default} size="50%" align="center" />
    * La configuration devrait maintenant être sauvegardée et vous êtes revenu à l'invite de commande
    * Entrer la commande ci-dessous pour commencer la compilation, cela peut prendre un certain temps
    ```bash
    make -j4
    ```
        <ImageView image={require('@site/docs/General/dfu-flash/img/make.webp').default} size="50%" align="center" />
    * Si la sortie finale ressemble à ceci, la compilation a réussi :

        ```bash
        Linking out/klipper.elf
        Creating bin file out/klipper.bin
        ```

## Vérifier si le mode de programmation est activé

    <LS />

## Programmer le firmware

    <CANBOOT />

...
* Choose `3z` to ensure that the `printer.cfg` has the `z_tilt` configuration or choose `4z` to ensure that the `printer.cfg` has the `quad_gantry_level` configuration. For independent dual Z-axis, select `3z`. [Configuration reference](https://mellow.klipper.cn/docs/category/klipper-%E5%8F%82%E8%80%83%E9%85%8D%E7%BD%AE)

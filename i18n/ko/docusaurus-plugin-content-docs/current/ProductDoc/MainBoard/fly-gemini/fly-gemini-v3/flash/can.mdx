...
:::
    ...
    <Tabs>
        <TabItem value="linux" label="dfu 플래시">
            *아래 이미지에 따라 점퍼 캡을 `boot1`에 연결한 후 `rst`를 눌러 보드를 재부팅합니다
            ...
            *보드가 dfu 모드에 들어갔는지 확인하려면 아래 명령어를 입력하세요
            ```bash
            lsusb
            ```
            *입력 후 `Enter` 키를 누르면 아래 이미지의 내용이 출력되며, 이미지에 있는 내용이 표시되면 보드가 성공적으로 dfu 모드에 진입한 것입니다
            ...
            * 아래는 dfu 플래시 명령어입니다
                ...
                ```
            5. 아래 이미지의 화살표 부분에 ``File downloaded successfully``가 표시되면 플래시가 성공적으로 완료된 것입니다

            :::warning

            빨간색 화살표가 가리키는 **뒤의 오류 메시지**는 무시하세요!!!
            ...
            플래시가 완료되면 보드의 점퍼 캡을 제거하고 `rst` 버튼을 눌러 보드를 재부팅하세요

...
# Eddy Usage
    ...
    * When calibrating the Eddy frequency, keep the coil PCB at least 5 mm away from the heat bed.
    * Please ensure the entire heat bed is at room temperature during testing, and do not turn on the heat bed during the test.
...
grep "LDC1612_FREQ = 40000000" ~/klipper/klippy/extras/ldc1612.py
  ```
    If the replacement is successful, a line containing `LDC1612_FREQ = 40000000` will be returned. If nothing is returned, the replacement was not successful.
    Or use this command to check if the modification was successful
...
### bed_mesh Configuration
    ...
    * For the `horizontal_move_z` parameter in [bed_mesh], it is recommended to set it to 2, so that Eddy can be as close to the platform as possible during scanning.

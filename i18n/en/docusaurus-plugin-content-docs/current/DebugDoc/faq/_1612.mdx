## Operating Instructions
    ...
    * For multi-Z-axis machines, a manual leveling should be performed once. After completing all calibrations, perform the leveling again and then re-do the calibrations.
    ...
    * Please use the `FUIDD` web page to add the following configuration and then perform the subsequent operations.
        ```cfg
        [force_move]
    ...
    ### Second step: Calibrate Eddy frequency
        ...
        * The reason for keeping it 5 mm away here is to facilitate the next step of doing the paper pull to obtain `z_offset`.
        ...
        * After the calibration is completed, `SAVE_CONFIG` to save the parameters.
        :::info
        * For printers with `z_tilt` or `quad_gantry_level (QGL)` functions, adjust the parallelism first. Adjust the parallelism of the X-axis for i3, the parallelism of the heated bed for the trident, and the parallelism of the gantry for voron 2.4. Then calibrate the Eddy frequency again.
        :::
    ### Third step: Temperature compensation
        ...
        * If the actual temperature of the 3D printer cavity is higher, you can appropriately increase the target temperature.
        * Please note that the higher the target temperature, the longer the calibration time will be, because Eddy needs more time to reach the temperature balance.

        :::
    
        1. After performing this operation, the UI will display the Z-axis adjustment box. Please use the manual [Z-axis offset calibration (Paper Test) method mentioned above](https://www.klipper3d.org/Bed_Level.html#the-paper-test) to sandwich the paper between the nozzle and the bed surface, and then confirm the value.
        2. After accepting the value, set the heated bed temperature to the maximum and the nozzle temperature to 220°C.
        3. If you are in a room with air conditioning or an open window, in order to ensure the temperature rise of Eddy, it is recommended to turn off the air conditioning or close the window, because the wind will affect the temperature rise.
        ...
        5. Repeat the manual Z-offset calibration (Paper Test) until the calibration is completed. If it is found that the Eddy temperature no longer rises, the following related commands can be used to end the calibration in advance.
        
        * Additional gcode commands available during the drift calibration include: TEMPERATURE_PROBE_NEXT
            * `TEMPERATURE_PROBE_NEXT` is used to force Eddy to sample new data before reaching the target temperature of `56°C` set by EDDy.
        ...
        * After completing these steps, Eddy will be able to provide an excellent first-layer printing effect in a wide temperature range!
...
* Second step: Enter `fluidd` `mainsail` through the printer's IP to find the dashboard `Movement Control` z-axis offset to continue fine-tuning the z_OFFSET and remember the fine-tuned value.
...
FIRMWARE_RESTART
```
* Fourth step: Adjust the `z - offset` through the following command. The UI interface that pops up allows you to adjust the `z - offset`. The value to be adjusted is the value adjusted during the printing process.
```bash
G28
PROBE_EDDY_CURRENT_CALIBRATE CHIP=fly_eddy_probe
```
...
:::info
 * If the above operations are completed but the bed scanning fails at the beginning, change the z limit to a virtual limit.
:::

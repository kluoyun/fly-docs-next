...
## ビルドの開始
    ...
    * 今、設定が保存されてコマンドラインのインターフェースに戻るはずです
...
## ファームウェアの書き込み
    <Tabs>
        <TabItem value="linux" label="dfu書き込み">
            * 以下の画像に従って、ジャンパーピンの`boot1``boot1`を挿し、`rst`を押してメインボードをリセットしてください
            ...
            * メインボードがdfuモードに入ったか確認するために以下のコマンドを入力します
            ```bash
            lsusb
            ```
            * 入力後、`Enter`キーを押すと、以下の画像の内容が表示され、画像内の内容が表示された場合、メインボードがdfuモードに入ったことを示します
            ...
            * 下記はdfu書き込みコマンドです
                ...
                ```
            5. 下図の矢印の位置に`File downloaded successfully`と表示されれば書き込み成功です

            :::warning

            赤い矢印で指された行の**後のエラー**は無視してください！！！
            ...
            書き込み完了後、メインボードのジャンパーピンを取り外し、`rst`を押してメインボードをリセットしてください

...
## Загрузка прошивки
    ...
    * Перед загрузкой прошивки подключите плату к UTOC или материнской плате с прошивкой моста с помощью кабеля XT30, ниже представлена схема подключения
        <ImageView image={require('@site/docs/General/get-id/img/utoc.png').default} size="80%" align="left" />
    * Убедитесь, что при выключенном питании сопротивление между CAN H и CAN L составляет около `60Ω`. Если оно равно `120Ω`, подключите перемычку на `120Ω`, если около `40Ω`, перемычку нужно убрать
...
## Проверка запуска прошивки
    ...
    * Если прошивка запустилась успешно, загорится светодиод

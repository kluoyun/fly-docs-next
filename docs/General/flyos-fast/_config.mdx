...
- 如果是普通配置项在系统网页配置页面修改后重启系统即可生效。
    ...
    - 打开`sys-config.conf`文件，将需要修改的配置项修改后 `保存` -> `关闭` 然后重启即可。
...
| 配置项 | 默认值 | 可使用值 | 说明 |
| :---: | :-------: | :--- | :--- |
| `board` |  <code>{props.board}</code> |   <code>{props.board}</code> |主板型号，请不要修改可使用值之外的配置 |
| `localtime` | `Asia/Shanghai` | [世界时区对照表](/docs/DebugDoc/flyos-fast/timezone) | 设置系统默认时区，系统在联网后会自动校时 |
| `canbus_bitrate` | `1000000` | `1000000`<br/> `500000`<br/> `250000` | 设置FAST系统所有can设备的默认速率，只能在此修改，默认为1M |
...
| 配置项 | 默认值 | 可使用值 | 说明 |
| :---: | :-------: | :--- | :--- |
| `screen` | `hdmi` | <code>nano</code>,<code>{props.screen}</code> <code>{props.screen3}</code> <br/> <code>{props.screen1}</code><code>{props.screen2}</code><br/> <code>{props.screen4}</code><code>{props.screen5}</code>| 设置屏幕，none则关闭所有显示 |
...
| `canbus_bitrate` | `1000000` | `1000000`, `500000`, `250000` | 设置FAST系统所有can设备的默认速率，只能在此修改，默认为1M |
...
| 配置项 | 默认值 | 可使用值 | 说明 |
| :---: | :-------: | :--- | :--- |
| `shutdown_pin_state` |<code>{props.state}</code>| `0`, `1` | 设置关机按键的电平状态<br/> 1: 低电平重复<br/> 0: 高电平触发  |
| `shutdown_pin` | <code>{props.shutdown_pin}</code>| <code>{props.shutdown_pin}</code> | 设置关机按键的触发引脚，设置none不启用  |
...
| 配置项 | 默认值 | 可使用值 | 说明 |
| :---: | :-------: | :--- | :--- |
| `firmware` | `空` | 有效的固件文件路径| 设置开机自动烧录固件, 目前仅C8、Gemini-V3与C5有效 |

...
<Tabs>
    ...
    * Please prepare a data cable with data transmission capability. If the length is particularly long, it is recommended to use a cable with a magnetic ring.
    * Before searching for the ID, use an XT30 data cable to connect the tool board to the Super5 PRO motherboard that has been flashed with bridge firmware.
    * And please connect the `120Ω` jumper, note that this is two jumpers. Below is a reference wiring diagram.

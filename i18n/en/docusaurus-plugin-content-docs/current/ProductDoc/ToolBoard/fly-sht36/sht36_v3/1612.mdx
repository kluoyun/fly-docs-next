...
# Eddy Usage
    ...
    :::info
    * When installing Eddy, the position of the bottom with respect to the nozzle should be at least 1 - 2 mm above the nozzle.
...
##### Add the following configuration to the `printer.cfg` configuration file to debug Eddy.
    ...
    ## Networked Bed Instructions
        ...
        * Add the following macro to the `printer.cfg` configuration.
            ...
            ```
        * For the local area network bed instruction, it is necessary to ensure that Klipper can use the object exclusion function. It is recommended to add it to the startup macro.
            ```bash
...
## Anti-collision Macro
    ...
    ### `[quad_gantry_level]` Bed Probing Optimization
        * If `[quad_gantry_level]` is configured, the following macro can be added to the configuration. If not, it is not necessary to add it.
    ...
    ### `[z_tilt]` Bed Probing Optimization

        * If `[z_tilt]` is configured, the following macro can be added to the configuration. If not, it is not necessary to add it.
        * This macro will reduce the probing height as the gantry height difference decreases.
            ```cfg
    ...
    ### Eddy is likely to be interfered by the heated bed during bed probing optimization macro

        * If a high-power AC heated bed (above 500W) is used.
        * The following macro is to turn off the heated bed during bed probing and reheat it after the bed probing is completed.
            ```cfg
            [gcode_macro BED_MESH_CALIBRATE]
            rename_existing: _BED_MESH_CALIBRATE
            gcode:

...
## Start Compilation
    ...
    * Press `Q` key, then a **Save configuration** prompt will appear, press `Y` key again
...
## Burn Firmware
    ...
    * Ensure that the resistance between CAN H and CAN L is around `60Ω` when measured with a multimeter in a power-off state; if it's `120Ω`, turn on the `120Ω` DIP switch; if it's around `40Ω`, turn off the DIP switch

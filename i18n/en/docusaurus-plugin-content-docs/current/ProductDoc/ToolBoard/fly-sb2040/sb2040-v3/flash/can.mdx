...
## Start Compilation
    ...
    * Press `Q`, then a **Save configuration** will appear, press `Y` again
...
## Burn Firmware
    ...
    * Please ensure that the resistance between CAN H and CAN L is around `60Ω` when measured with a multimeter under power-off conditions. If it is `120Ω`, the `120Ω` DIP switch needs to be turned on; if it is around `40Ω`, the DIP switch needs to be turned off.

---
sidebar_position: 4
sidebar_label: RS232 펌웨어 번인
---

import USBFLASH from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/dfu-flash/_firmware/2040/_rp2.mdx';
import MakeOperate from "/docs/General/klipper-make/_make-operate.mdx";
import RS232 from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/dfu-flash/_firmware/2040/_rs232.mdx';
import SSH from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/dfu-flash/_firmware/_ssh.mdx';

import LS from '@site/i18n/ko/docusaurus-plugin-content-docs/current/General/dfu-flash/_uf2.mdx';

# RS232 펌웨어 컴파일

## 컴파일 시작

    <SSH />

    <MakeOperate/>

    <RS232 />

    * 마이크로컨트롤러 시작 시 GPIO 핀을 설정하려면, 입력: !gpio5 (주의: !는 대문자)
        <ImageView image={require('@site/docs/ProductDoc/ToolBoard/fly-sb2040/sb2040-v3/flash/img/rs232.png').default} size="100%" align="center" />
    * `Q` 키를 누르고, **Save configuration**이 나타나면 `Y` 키를 다시 누르세요
        <ImageView image={require('@site/docs/General/dfu-flash/img/configuration.png').default} size="50%" align="center" />
    * 이제 설정이 저장되고 명령줄 인터페이스로 돌아갑니다
    * `make -j4`를 입력하여 컴파일을 시작합니다. 시간이 좀 걸립니다
        <ImageView image={require('@site/docs/General/dfu-flash/img/make.png').default} size="50%" align="center" />
    * 마지막으로 아래 내용이 출력되면 컴파일이 성공한 것입니다

        ```bash
        Linking out/klipper.elf
        Creating uf2 file out/klipper.uf2
        ```


## 번인 모드 진입

    :::tip

    왼쪽이 `RESET 버튼`, 오른쪽이 `BOOT 버튼`입니다

    :::

    * DFU 모드로 진입하는 방법은 두 가지가 있습니다
    * 첫 번째 방법: 메인보드의 전원을 완전히 차단한 상태에서 BOOT 버튼을 누른 상태로 데이터 전송이 가능한 데이터 케이블의 한쪽 끝을 메인보드에, 다른 쪽 끝을 호스트 컴퓨터에 연결합니다
    * 두 번째 방법: 데이터 전송이 가능한 데이터 케이블의 한쪽 끝을 메인보드에, 다른 쪽 끝을 호스트 컴퓨터에 연결한 후, BOOT 버튼을 누르고 RST 버튼을 누른 후 RST 버튼을 놓고 마지막으로 BOOT 버튼을 놓습니다
        <ImageView image={require('@site/docs/ProductDoc/ToolBoard/fly-sb2040/sb2040-v3/flash/img/boot.png').default} size="100%" align="center" />

## 번인 모드 진입 확인

    <LS />


## 펌웨어 번인 
    
    <USBFLASH />
    


## 펌웨어가 시작되었는지 확인

    * 펌웨어가 정상적으로 시작되면 LED가 켜집니다
        <ImageView image={require('@site/docs/ProductDoc/ToolBoard/fly-sb2040/sb2040-v3/flash/img/led.png').default} size="100%" align="center" />


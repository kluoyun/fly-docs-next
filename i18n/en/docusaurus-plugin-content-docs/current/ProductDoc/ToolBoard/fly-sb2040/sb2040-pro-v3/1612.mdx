...
# Eddy Usage
    ...
    * Please note that it is recommended to use a separate Z limit switch for bed leveling, and not to use the bed leveling sensor as the Z limit switch.
    * When installing Eddy, the bottom should be at least 1-2 mm above the nozzle.
    * When calibrating Eddy's frequency, ensure the coil PCB is at least 5mm away from the heatbed.
    * Please ensure the entire heatbed is at room temperature during testing, and do not turn on the heatbed during the test.

    :::

## Wiring Diagram

    * Trademark facing down
...
### Modifying the default frequency of ldc1612 1612
    ...
    Use the Mobaxterm tool to execute the commands
...
grep "LDC1612_FREQ = 40000000" ~/klipper/klippy/extras/ldc1612.py
  ```
    If the replacement is successful, a line containing `LDC1612_FREQ = 40000000` will be returned. If there is no return, the replacement was unsuccessful.
...
```
   ...
   Tip: After modification, please use this command to restart the Klipper service and recalibrate Eddy's frequency
   ```cfg
...
### Bed Mesh Configuration
    ...
    * In [bed_mesh], set the `horizontal_move_z` parameter to 2, allowing Eddy to be as close to the platform as possible during scanning.

...
# USB Firmware Compilation
    ...
    * The bootloader has been pre-flashed on the TF card at the factory, so there is no need to re-flash it.
...
## Start Compilation
    ...
    * Press `Q` key, and when **Save configuration** appears, press `Y` again.
    ...
    * If the output ends with the following content, then the compilation is successful.

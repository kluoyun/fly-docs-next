---
sidebar_position: 3
sidebar_label: 듀얼 x축/인덱스 참고 구성
---

## slicer 시작 코드는哔哩哔哩up主 muimoon女生 提供:
```bash
...
# 도우미 스크립트로 카리지 주차 (T0 및 T1 매크로에서 호출됨)
[gcode_macro PARK_extruder]
gcode:
	RESPOND MSG="Leftmost extruder parking"
...
[gcode_macro PARK_extruder1]
gcode:
	RESPOND MSG="Rightmost extruder parking"
...
# 도우미 스크립트, 필요하다면 변수 xoffset이 매크로 실행 중 변경될 수 있습니다
# 그리고 매크로 확장은 그 전에 발생하기 때문에
gcode:

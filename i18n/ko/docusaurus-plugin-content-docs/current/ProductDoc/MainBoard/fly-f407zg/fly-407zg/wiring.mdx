---
sidebar_position: 4
sidebar_label: 메인보드 배선
---




# 메인보드 배선

## 인터페이스 다이어그램

### 인터페이스 설명

    <ImageView image={require('@site/docs/ProductDoc/MainBoard/fly-f407zg/fly-407zg/img/interface.png').default} size="50%" align="center" />

----

### 핀 설명

    <ImageView image={require('@site/docs/ProductDoc/MainBoard/fly-f407zg/fly-407zg/img/pin.png').default} size="50%" align="center" />

---

## 전원 배선

    <ImageView image={require('@site/docs/ProductDoc/MainBoard/fly-f407zg/fly-407zg/img/wiring/power.png').default} size="40%" align="center" />
----

## 드라이버 설치

### 메인보드 드라이버 점퍼

* **STEP/DIR 모드.** 이 모드에서는 점퍼 캡을 사용하여 세분화를 설정하고, 전류를 조절하기 위해 포텐티오미터를 조정합니다. 가장 일반적인 A4988 드라이버는 이 모드를 사용하며, 드라이버 제조사가 제공하는 세분화 설정표를 참조하여 점퍼 캡으로 세분화를 설정해야 합니다.
* **UART 모드.** 이 모드를 사용하는 가장 일반적인 드라이버는 TMC2208, TMC2209, TMC2226 등입니다. 이러한 드라이버 칩은 UART 비동기 직렬 통신을 통해 메인 컨트롤러와 통신할 수 있으며, 설정 파일을 수정하여 드라이버의 세분화, 운영 전류, 조용 모드 등을 설정할 수 있습니다.
* **SPI 모드.** 이 모드를 사용하는 가장 일반적인 드라이버는 TMC5160, TMC2230 등입니다. 이러한 드라이버 칩은 SPI를 통해 메인 컨트롤러와 동기 직렬 통신을 하며, 마찬가지로 설정 파일을 수정하여 드라이버의 세분화, 운영 전류, 조용 모드 등을 설정할 수 있습니다.

    <ImageView image={require('@site/docs/ProductDoc/MainBoard/fly-super/fly-super8/img/wiring/drive-dip.png').default} size="40%" align="center" />

----

### 드라이버 설치

* 드라이버 설치 전에 적절한 검사를 해야 하며, 드라이버나 메인보드에 손상을 주지 않도록 주의해야 합니다.
* **FLY 드라이버:** 무한위치 원점 복귀 기능을 사용하지 않는 경우, 디프 스위치를 1로 설정하세요; 반대로 무한위치 원점 복귀 기능을 사용하려면 디프 스위치를 ON으로 설정하세요.

    <ImageView image={require('@site/docs/ProductDoc/MainBoard/fly-super/fly-super8/img/wiring/fly2209.png').default} size="40%" align="center" />

    :::danger

    * **주의!!! 드라이버 모듈을 설치할 때, 반드시 EN 핀이 좌상단에 위치하도록 올바르게 삽입해야 합니다. 그렇지 않으면 드라이버나 메인보드가 손상될 수 있습니다!!! 드라이버에 방열판을 붙이는 것도 잊지 마세요!!!**

    :::

    <ImageView image={require('@site/docs/ProductDoc/MainBoard/fly-super/fly-super8/img/wiring/drive-install.png').default} size="40%" align="center" />


----
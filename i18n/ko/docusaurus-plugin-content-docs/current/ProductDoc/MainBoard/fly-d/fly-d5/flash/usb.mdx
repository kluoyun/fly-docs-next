...
## 플래시 모드 진입
    ...
    * 메인보드를 상위기기로 연결한 후 **리셋 버튼을 두 번 클릭**하면 메인보드에 LED가 깜빡입니다.
    :::warning 경고
       * LED가 깜빡이지 않으면 Katapult 펌웨어를 다시 플래시해야 합니다: <Button variant="contained" disableElevation href="/docs/ProductDoc/MainBoard/fly-d/fly-d5/flash/bl#進入dfu燒錄模式">Katapult 펌웨어 플래시</Button>

...
## 컴파일 시작
    ...
    * **GPIO 핀을 마이크로컨트롤러 시작 시 설정** 선택, 입력: **!gpio5**
    :::warning
    ...
    * 마지막에 아래의 내용이 출력되면 컴파일이 성공한 것입니다
...
## 프래싱 모드 진입
    ...
    * 첫 번째 방법: 보드 전체가 완전히 전원이 꺼진 상태에서 부트 버튼을 누른 후 데이터 전송용 케이블 한쪽을 보드에 다른 한쪽을 호스트에 연결합니다
    * 두 번째 방법: 데이터 전송용 케이블 한쪽을 보드에 다른 한쪽을 호스트에 연결한 후 부트 버튼을 누른 다음 리셋 버튼을 누른 후 리셋 버튼을 놓고 마지막으로 부트 버튼을 놓습니다
